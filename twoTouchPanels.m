...
try
	...
	for iTrial=1:trialN
		...
		for jObj = 1: nObjects
			if resultsFron == ture & resultsBack== false
				textMonkey = ['front monkey win: ' upper(stims{jObj})];
				...
				fronTouch = true;
				break;
			elseif resultsFron == false & resultsBack== true
				textMonkey = ['back monkey win: ' upper(stims{jObj})];
				eval([stims{jObj} 'Reward = true;'])
